...
class MpcSinglePendulum:
    ...
    def solve(self, x_init, N, X_guess = None, U_guess = None): 
        ...
        ## ==> Terminal cost 
        state      = [q[N-1], v[N-1]]
        state_norm = (state - conf.sc_mean) / conf.sc_scale
        #print(state, state_norm)
        ...
        if conf.TC_on:
            ...
            self.opti.subject_to(nn_output > (1 - 5e-7))
...
if __name__ == "__main__":
    ...
    for i in range(conf.mpc_step):
        ...
        try:
            ...
            if "Infeasible_Problem_DetSected" in str(e):
                print("\n########################################")
        ...
        array.append(np.array(sol.value(mpc.q)))
